/**
 * Markdown 解析器的测试（M1–M4）。
 *
 * 与 `SessionMirrorTests` / `UsageStateTests` / `TranscriptAssemblerTests` 同一手法：
 * 解析器是纯状态机，不 import SwiftUI / UIKit、不碰网络、不碰磁盘 —— 直接喂源文、
 * 断言块数组。所以「块级识别、行内接入、尾部冻结、未闭合围栏」这四处最容易错的判断
 * 都能离线验。
 *
 * 判据与出处：docs/dev/plans/M6-presentation-layer.md §五（M1–M4）。
 * 流式语义照**运行时产物**的上游实现（`@deepseek-ai/dsh-client-ui-primitives` 的
 * `markdown/incremental.ts`）：除尾部外全部冻结，冻结块的键是源偏移。
 */
import Foundation
...
struct MarkdownParserTests {
    ...
    private static func tailFreezing() {
        ...
        // M3a：按任意切分喂入，最终结果与全量解析逐块相同 —— 增量正确性的地基。
        //      这里用最狠的切法：逐字符。
        ...
        // M3e：重扫起点 = 冻结边界之后那一块的源偏移，而**不是** 0 —— 这就是「只重解析尾部」。
        //      （取不到尾块时用 -1，让断言如实失败而不是越界崩掉 —— 红阶段要能看清是哪些条红。）
